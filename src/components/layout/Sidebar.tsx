...
const PRIORITIES = [
  ...
  { id: 'important', label: 'Important', color: '#fb923c' },
    ...
    <div className="flex flex-col h-full select-none">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-3">
        ...
        <div className="border-t pt-2 space-y-0.5">
          <SectionHeader label="Priorities" sectionKey="priorities" />
          {(sectionOpen.priorities ?? true) && PRIORITIES.map(p => (
            <button
              key={p.id}
              ...
              className={cn(
                ...
                selectedView === 'priority' && selectedPriority === p.id && 'bg-accent font-medium text-primary',
              )}
            >
              <Flag size={16} className="flex-shrink-0" style={{ color: p.color }} />
              <span>{p.label}</span>
            </button>
          ))}
        </div>
